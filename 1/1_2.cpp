...
struct Large{
  Large(){
    num = new long[MAXDIGIT / NUMOFPUSH + 1];
    memset(num, 0, sizeof(long) * (MAXDIGIT / NUMOFPUSH + 1));
    len = 0;
  }
  Large(int l){
    num = new long[l];
    memset(num, 0, sizeof(long) * l);
    len = 0;
  }
  ~Large(){
    delete[] num;
...
int main(){
  ...
  for (int i = 0; i < n; ++i){
    ...
    for (int j = la - 1; j >= 0; --j){
      ...
      if (num.length() >= NUMOFPUSH || j == 0) {
        a.num[a.len++] = atoll(num.c_str());
        num.clear();
      }
    }
        ...
        if (flag) {
          if (k == c.len - 1) {
            ...
            if (c.num[k] / 1 == 0) 
              ...
              printf("000%ld", c.num[k]);
            else if (c.num[k] / 100 == 0) 
              printf("00%ld", c.num[k]);
            else if (c.num[k] / 1000 == 0) 
              printf("0%ld", c.num[k]);
            //else if (c.num[k] / 10000 == 0)
            //  printf("0%ld", c.num[k]);
            //else if (c.num[k] / 100000 == 0)
            //  printf("0%ld", c.num[k]);
            //else if (c.num[k] / 1000000 == 0)
            //  printf("00%ld", c.num[k]);
            //else if (c.num[k] / 10000000 == 0)
            //  printf("0%ld", c.num[k]);
            else
              printf("%ld", c.num[k]);
          }          
